...
<head>
    ...
    <title>{{ $title ?? 'Newsletter Archive' }} | UH Population Health</title>
        ...
        <header class="bg-uh-red text-white shadow-sm">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div class="flex flex-col space-y-2">
                    <h1 class="text-3xl font-bold">{{ $title ?? 'Newsletter Archive' }}</h1>
                                ...
                                <div class="hidden md:flex space-x-2">
                                    @foreach ($campaigns->getUrlRange(1, $campaigns->lastPage()) as $page => $url)
                                        @if ($page == $campaigns->currentPage())
                                            <span class="relative z-10 inline-flex items-center px-4 py-2 border border-red-500 bg-red-50 text-sm font-medium text-red-600">
                                                {{ $page }}
                                            </span>
                                        @else
